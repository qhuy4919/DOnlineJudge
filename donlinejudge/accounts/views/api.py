...
from accounts.serializers import (
    ...
    ProfilePageNoPasswordSerializer,
...
class RegisterAPI(APIView):
    ...
    def post(self, request):
        ...
        if username != "" and username.isalnum():
            ...
            return response_bad_request(
                {"username": "Username can only contain alphanumeric characters."}
            )
        if email != "":
            try:
        ...
        return response_bad_request(
            {
                "password": "The password length must be more than 8 with letters and numbers."
            }
        )
        ...
        if serializer.is_valid():
            ...
            if request.user.is_authenticated:
                if user != request.user:
                    return response_bad_request(
                        "Cannot login as another user if you are already logged on"
                    )
            ...
            return response_ok(
                {
                    ...
                    "token": str(token),
                    "TTL": int(
                        TOKEN_EXPIRE_AFTER_SECONDS
                        - (utc_now() - token.created).total_seconds()
                        + 0.5
                    ),
                }
            )
            ...
            return response_bad_request(
                {"username": "Username or Password is incorrect. Try again."}
            )
...
class ChangePasswordAPI(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if data.get("new_password1", "") != "" and data.get(
                "new_password1", ""
            ...
            return response_bad_request(
                {"entered_password": "The two password fields didn't match."}
            )
...
class OwnProfilePageAPI(generics.GenericAPIView):
    ...
    def put(self, request):
        ...
        if data.get("password", "") != "":
            return response_bad_request(
                "Please use profile/change-password API for password changing"
            )
        ...
        if data.get("profile_pic", "") != "":
            if IEV.validate(str(data["profile_pic"])):
                ...
                return response_bad_request(
                    {
                        "profile_pic": "The file must have an image extension of .jpg, .jpeg or .png"
                    }
                )
        user.save()
        ...
        try:
            ...
            if not os.path.exists(filepath):
                ...
                return response_no_content(
                    "Your avatar file could not be located. Resetting it to default."
                )
...
class ProfilePageAPI(APIView):
    def get(self, request, id):
        ...
        return response_ok(
            ProfilePageNoPasswordSerializer(User.objects.get(id=id)).data
        )
    ...
    def post(self, request):
        ...
        if gen_type == "list":
            ...
            for i in range(howmany):
                ...
                created_users.append(User(username=un, password=randstr(length=10)))
            User.objects.bulk_create(created_users)
            return response_created(
                UserCreationSerializer(created_users, many=True).data
            )
            ...
            return response_bad_request(
                "Missing 'user_gen_type' in body or type is unsupported"
            )
    ...
    def put(self, request, id):
        ...
        if data["admin_type"] != "":
            if data["admin_type"] not in AdminType.TYPES:
                return response_bad_request(
                    "admin_type '{}' is not valid.".format(data["admin_type"])
                )
        ...
        if data["is_staff"] != "":
            if data["is_staff"] not in ["true", "false"]:
                return response_bad_request(
                    "is_staff '{}' should be either 'true' or 'false'.".format(
                        data["is_staff"]
                    )
                )
        ...
        if data["problem_permission"] != "":
            if data["problem_permission"] not in ProblemPermission.TYPES:
                return response_bad_request(
                    "problem_permission '{}' is not valid.".format(
                        data["problem_permission"]
                    )
                )
